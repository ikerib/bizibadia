...
#[AsCommand(
    name: 'app:import:04-eguraldia',
    description: 'Add a short description for your command',
)]
class Import04EguraldiaCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $eguraldiak = $this->getCsvAsArray($filename);
        $progressBar = new ProgressBar($output, count($eguraldiak));
        $progressBar->start();

        $udala = $em->getRepository(Udala::class)->findOneBy(['id' => 1]);

        foreach ($eguraldiak as $e) {
            $egu = new Eguraldia();
            $egu->setUdala($udala);
            $egu->setIzena($e['izena']);
            $egu->setOldid($e['id']);
            $em->persist($egu);
            $progressBar->advance();
        }
        $em->flush();
        $io->success('Eguraldiak inportatu dira.');
